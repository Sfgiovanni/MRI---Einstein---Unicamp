"""
Teste 3A - Classificadores sobre volume hipocampal (esquerdo/direito/total, normalizado
por eTIV), mesmo protocolo de CV 5-fold por sujeito dos outros metodos.
"""
import argparse
import os
...
def main():
    ...
    features_parquet = args.features_parquet or f"features/{args.dataset}_hippocampus_features.parquet"
    ...
    print(f"[{args.dataset}] hippocampus: {len(features_df)} subjects x {len(feature_cols)} dims ({feature_cols})")
    run_cv(features_df, folds_df, feature_cols, "hippocampus", output_dir)
